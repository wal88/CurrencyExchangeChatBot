...
exports.ocrImage = function (session, from, to, attachment) {
    ...
    // test because bot emulator's localhost URL doesn't work with OCR api
...
function overlayConvertedAmounts(session, amounts) {
    ...
    //here we specify the new text overlay for each number, pushing each overlay JSON object into the 'transformations' array
    for (let amount of amounts) {
        let bounds = amount.boundingBox;
        transformations.push({
            overlay: "text:dark_text:" + amount.text.join(" "),
            gravity: "north_west",
            x: bounds[0], y: bounds[1],  //, width: bounds[3], height: 60 // dont need as we adjust with font size
            angle: angle
        });
    }
